    ...
    <div className={`${styles.boxWidth} my-4 px-4`}>
        {collectionShopify ? (
            <>
                ...
                <h2 className='text-[20px] text-gray-900 mb-2'>{collectionShopify?.description}</h2>
                    ...
                    {collectionShopify?.products?.nodes.map((product, index) => (
                      <li key={index} className={`flex sm:justify-between border-b-2 py-2 flex-col ${(index + 1) % 2 === 0 ? 'sm:flex-row-reverse' : 'sm:flex-row'} `}>
                        ...
                        <div className={`${(index + 1) % 2 === 0 ? 'sm:pr-6' : 'sm:pl-6'} my-2 sm:my-0 w-[100%] h-[100%]`}>
                          <span className='text-[24px] font-secondary font-bold uppercase block'>{product.title}</span>
                ...
                <SEO
                  ...
                  titleSeparator=' - '
                  description={collectionShopify?.description || 'Cargando...'}
